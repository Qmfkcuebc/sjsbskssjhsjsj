...
print ("welcome, Created By Shah Falsafi (TG TeriPHP)\n")

print ("Please subscribe to the channel to receive updates! : for telegram : @TeriPHP\n")
...
# created By Shah & iLiyaFalsaFi(TeriPHP)(TG TeriPHP)(libs for kabbalah)
				...
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								bot.sendMessage(target, "سلام تو سازنده من هستی و تو (TeriPHP) رهبر خانواده فلسفی هستی", message_id=msg.get("message_id"))
							except:
								print("err admin")
							...
							try:
								bot.sendMessage(target, "صــلام لاوم😍🤍", message_id=msg.get("message_id"))
							except:
								print("err answer hello")
								
						elif msg.get("text").startswith("خوبی") or msg.get("text").startswith("خبی"):
							try:
								bot.sendMessage(target, "فداتشـمم تو دوبی😉💛", message_id=msg.get("message_id"))
							except:
							...
							try:
								bot.sendMessage(target, "خنده‍ هاي دلبرمـو عشقـــههه🥺💜", message_id=msg.get("message_id"))
							except:
								print("err luagh")
								
						elif msg.get("text") == "😐":
							try:
								bot.sendMessage(target, "برلیم؟", message_id=msg.get("message_id"))
							except:
							...
							try:
								...
								bot.sendMessage(target, "پیام مورد نظر با موفیقت سنجاق شد😍💫", message_id=msg.get("message_id"))
							except:
								print("err pin")
							...
							try:
								...
								bot.sendMessage(target, "پیام مورد نظـر با موفقـیت از سنجاق برداشته شـد😉🤙", message_id=msg.get("message_id"))
							except:
								print("err unpin")
							...
							try:
								...
								bot.sendMessage(target, "فونتـای خوشملمو پیویت فرستـادمم🥺🤎", message_id=msg["message_id"])
							except:
								bot.sendMessage(target, "دستور رو درست وارد کن دیگه😁", message_id=msg["message_id"])
					...
					if data["type"]=="RemoveGroupMembers":
						...
						try:
							user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
							bot.sendMessage(target, f "{user} عزیـز به گپ {name} خوش اومدی 🥰❤ کانال سازنده @TeriPHP"])
							# bot.deleteMessages(target, [msg["message_id"]])
						except:
						...
						try:
							user = bot.getUserInfo(data['performer_object']['object_guid'])["data"]["user"]["first_name"]
							bot.sendMessage(target, f"هــی {user}ی🌹 \n • به  گروه خیلی خوش اومدیییی  ایدی سازنده  @ TeriPHP", message_id=msg["message_id"])
							# bot.deleteMessages(target, [msg["message_id"]])
						except:
